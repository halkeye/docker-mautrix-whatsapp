FROM curlimages/curl:8.22.0 AS builder
ARG TARGETPLATFORM
ARG UPSTREAM_VERSION=v0.2609.0
...
COPY --from=mwader/static-ffmpeg:9.0.2 /ffmpeg /usr/local/bin/
COPY --from=mwader/static-ffmpeg:9.0.2 /ffprobe /usr/local/bin/
